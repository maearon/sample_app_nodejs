...
export function initializeSocket(
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    console.log(`🔌 [src/socket/socket.ts] User connected: ${socket.userEmail} (${socket.id})`);
    ...
    socket.on('disconnect', () => {
      console.log(`🔌 [src/socket/socket.ts] User disconnected: ${socket.userEmail} (${socket.id})`);
      return;
    });
  });
}
